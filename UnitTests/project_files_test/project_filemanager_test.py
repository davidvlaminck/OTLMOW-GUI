...
def test_get_project_from_dir_given_project_dir_location():
    project_dir_path = PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1'

    project = ProjectFileManager.get_project_from_dir(project_dir_path)

    assert project.project_path == Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1')
    assert project.subset_path == Path(
        PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1' / 'OTL_AllCasesTestClass.db')
    assert project.assets_path == Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1' / 'assets.json')
...
def test_get_project_from_dir_project_dir_missing():
    project_dir_path = PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_missing'
...
def test_get_project_from_dir_details_missing():
    project_dir_path = PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_details_missing'
...
def test_save_project_given_details(mock_get_otl_wizard_projects_dir):
    project = Project(
        project_path=Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_2'),
        subset_path=Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1' / 'OTL_AllCasesTestClass.db'),
        assets_path=Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_2' / 'assets.json'),
    ...
    project_dir_path = PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_2'
...
def test_get_all_otl_wizard_projects(caplog, mock_get_otl_wizard_projects_dir):
    ...
    assert projects[0].project_path == Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1')
...
def test_export_project_to_file():
    project = Project(
        ...
        bestek="bestek",
        laatst_bewerkt=datetime.datetime(2023, 11, 1))
    file_path = Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_1.otlw')
...
def test_load_project_file():
    ...
    os.remove(file_path)
    shutil.rmtree(Path(PARENT_OF_THIS_FILE / 'mock_otlwizard_project_dir' / 'project_extract_and_load'))
