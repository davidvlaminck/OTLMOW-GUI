...
"""
from PyQt6.QtWidgets import QFileDialog, QDialog, QApplication, QWidget, QMainWindow, QVBoxLayout, \
    QHBoxLayout, QFormLayout, QComboBox, QPushButton, QInputDialog, QLineEdit, QLabel
import sys
import os
...
if __name__ == "__main__":
    ...
    qgv.new(Dot(Graph("Main_Graph"), show_subgraphs=show_subgraphs, font=QFont("Arial", 12),
                margins=[20, 20]))
    # Define sone graph
    ...
    n5 = qgv.addNode(qgv.engine.graph, "Node5", label="N5", shape="polygon", fillcolor="red",
                     color="white")
    n6 = qgv.addNode(qgv.engine.graph, "Node6", label="N6", shape="triangle",
                     fillcolor="blue:white:red")

    sub = qgv.addSubgraph(qgv.engine.graph, "sub graph", qgv.engine.graph.graph_type,
                          label="Subgraph", fillcolor="blue:white:red")
    ...
    def add_subgraph():
        ...
        if dlg.OK and dlg.subgraph_name != '':
            qgv.addSubgraph(qgv.engine.graph, dlg.subgraph_name,
                            subgraph_type=GraphType.SimpleGraph, label=dlg.subgraph_label)
            qgv.build()
