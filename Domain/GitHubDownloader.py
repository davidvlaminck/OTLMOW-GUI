...
class GitHubDownloader:
    def __init__(self, repo_owner_and_name: str, timeout: int = 60):
    ...
    def get_repo(self):
        if self.repo is None:
            self.repo = Github(timeout=self.timeout).get_repo(self.repo_owner_and_name)
        return self.repo
    ...
    def download_full_repo(self, destination_dir: Path):
        ...
        resp = requests.get(download_zip_url, timeout=self.timeout)
